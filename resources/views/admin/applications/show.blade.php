<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Detail Lamaran (Admin)') }}</h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div><dt class="text-gray-500">Status Saat Ini</dt><dd class="font-semibold text-gray-900 uppercase">{{ $application->status }}</dd></div>
                </dl>
                ...
                @if($application->resume_path)
                    <div class="mt-6 flex flex-wrap gap-4">
                        <a href="{{ route('admin.applications.resume', $application) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-semibold text-sm">
            ...
            <div class="flex gap-4">
                <a href="{{ route('admin.applications.index') }}" class="text-gray-600 hover:text-gray-900 font-semibold text-sm"><i class="fas fa-arrow-left mr-1"></i> Kembali</a>
                <a href="{{ route('admin.evaluations.show', $application->id) }}" class="text-blue-600 hover:text-blue-800 font-semibold text-sm"><i class="fas fa-star mr-1"></i> Lihat / Beri Penilaian</a>
            </div>
        </div>
    </div>
</x-app-layout>
