        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                <div class="mb-6 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">Ubah Template: {{ $letterTemplate->name }}</h3>
                        <p class="text-sm text-gray-500">Sesuaikan kembali pengaturan dan isi template surat otomatis.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.letter-templates.update', $letterTemplate) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $letterTemplate->name) }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="type" name="type" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                <option value="acceptance" {{ old('type', $letterTemplate->type) === 'acceptance' ? 'selected' : '' }}>Penerimaan (Acceptance)</option>
                                <option value="rejection" {{ old('type', $letterTemplate->type) === 'rejection' ? 'selected' : '' }}>Penolakan (Rejection)</option>
                                <option value="interview_invitation" {{ old('type', $letterTemplate->type) === 'interview_invitation' ? 'selected' : '' }}>Undangan Interview (Interview Invitation)</option>
                            </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" id="subject" name="subject" value="{{ old('subject', $letterTemplate->subject) }}" required
                               class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="body" name="body" rows="12" required
                                  class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm">{{ old('body', $letterTemplate->body) }}</textarea>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="is_active" name="is_active" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            <option value="1" {{ old('is_active', $letterTemplate->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('is_active', $letterTemplate->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Non-aktif</option>
                        </select>
