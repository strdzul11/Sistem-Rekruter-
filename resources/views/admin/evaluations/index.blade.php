    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8"
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-bold text-gray-900">Daftar Lamaran Kerja</h3>
                    <p class="text-sm text-gray-500">Berikan penilaian kriteria pelamar untuk memproses perangkingan SAW.</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-100 text-gray-400 text-xs font-semibold uppercase tracking-wider">
                                ...
                                <th class="py-4 px-6">Tipe Aplikasi</th>
                        ...
                        <tbody class="divide-y divide-gray-100 text-gray-700 text-sm font-medium">
                            @foreach ($applications as $app)
                                <tr>
                                    ...
                                    <td class="py-4 px-6 uppercase text-xs tracking-wider text-gray-500">
                                        {{ str_replace('_', ' ', $app->application_type) }}
                                    </td>
                                    <td class="py-4 px-6">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium uppercase tracking-wide bg-blue-50 text-blue-700">
                                            {{ $app->status }}
                                        </span>
                                    </td>
                                    <td class="py-4 px-6">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium uppercase tracking-wide {{ $app->evaluations->count() > 0 ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700' }}">
                ...
                {{-- Modal Preview Cepat Penilaian SAW (Admin) --}}
                        ...
                        <div x-show="!previewLoading && !previewError && previewData" class="space-y-6">
                            <div class="flex justify-between items-center bg-gray-50 p-4 rounded-xl border border-gray-100">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-550 mt-0.5" x-text="previewData?.position + ' (' + previewData?.company + ')'"></p>
                                </div>
                                <div class="text-right">
                                    <span class="text-xxs text-gray-450 block font-bold uppercase tracking-wider">SKOR SAW TOTAL</span>
                                    <span class="text-2xl font-extrabold text-blue-600" x-text="previewData?.total_score"></span>
                                </div>
                            </div>
