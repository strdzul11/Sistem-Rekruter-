<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Form Penilaian') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if($breakdown->isNotEmpty())
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                            ...
                            <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <div class="text-right text-xs text-gray-500">
                                        <div>Bobot: <strong class="text-emerald-600">{{ $item->weight }}%</strong></div>
                    ...
                    <div class="mt-4 flex justify-end">
                        <div class="text-right bg-emerald-50 border border-emerald-200 px-5 py-3 rounded-xl">
                            <div class="text-xs text-emerald-600 font-semibold mb-0.5">Estimasi Skor SAW Saat Ini</div>
                            <div class="text-2xl font-extrabold text-emerald-700">{{ number_format($finalScore * 100, 2) }}%</div>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <form method="POST" action="{{ route('hrd.evaluations.store') }}" class="space-y-6">
                    @csrf
                    ...
                    @forelse($criteria as $index => $criterion)
                        ...
                        <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <span class="text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">Bobot {{ $criterion->weight }}%</span>
                            </div>
                    ...
                    @empty
                        <p class="text-gray-500 text-sm">Belum ada kriteria penilaian aktif. Hubungi Admin untuk mengatur kriteria SAW.</p>
                    @endforelse

                    @if($criteria->isNotEmpty())
                        <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                            <a href="{{ route('hrd.evaluations.index') }}" class="text-gray-600 hover:text-gray-900 font-semibold text-sm"><i class="fas fa-arrow-left mr-1"></i> Kembali</a>
                            <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 px-6 rounded-xl"><i class="fas fa-save mr-2"></i> Simpan Penilaian</button>
                        </div>
                    @endif
                </form>
            </div>
